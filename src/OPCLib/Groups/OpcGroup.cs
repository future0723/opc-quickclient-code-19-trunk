...
using OPCLib.Utility;
using MySqlHelper = OPCLib.Utility.MySqlHelper;
...
namespace OPCLib
{
    ...
    public delegate void DataChangeEventHandler(object sender, DataChangeEventArgs eventArgs);
    ...
    public delegate void ReadCompleteEventHandler(object sender, ReadCompleteEventArgs eventArgs);
    ...
    public delegate void WriteCompleteEventHandler(object sender, WriteCompleteEventArgs eventArgs);
    ...
    public delegate void CancelCompleteEventHandler(object sender, CancelCompleteEventArgs eventArgs);
    ...
    public class OpcGroup : IOPCDataCallback
    {
        ...
        internal OpcGroup(ref IOPCServer serverLink, bool isPublic, string groupName, bool setActive,
            int requestedUpdateRate)
        {
            m_IfServer = serverLink;

            m_State.Name = groupName;
            m_State.Public = isPublic;
            m_State.UpdateRate = requestedUpdateRate;
            m_State.Active = setActive;
            m_State.TimeBias = 0;
        ...
        {
            m_IfAsync.SetEnable(doEnable);
        }

        public void GetEnable(out bool isEnabled)
        {
            m_IfAsync.GetEnable(out isEnabled);
        }
                                            ...
                                            IntPtr errorsPtr)
        {
            ...
            {
                itemStates[i] = new OpcItemState {Error = Marshal.ReadInt32((IntPtr) runError)};
                runError += 4;
            ...
            try
            {
                var parameter = new MySqlParameter("?p1", MySqlDbType.Text, 0) {Value = json1};
